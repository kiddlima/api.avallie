...
service.getSupplier = getSupplier;
...
function getSupplier(token) {
    return new Promise((resolve, reject) => {
        authService.verifySupplier(token.split(' ')[1])
            .then((response) => {
                dao.findSupplierById(response._id)
                    .then((response) => {
                        resolve(response);
                    })
                    .catch((err) => {
                        reject(appHelper.buildResponseMessage(404, err));
                    })
            })
            .catch((err) => {
                reject(err);
