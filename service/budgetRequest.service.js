											...
											.then((response) => {
												...
												for (let j = 0; j < groupedMatches.length; j++) {
													...
													emailHelper.sendEmail(getToSupplierEmailInfo(groupedMatches[j], groupedMatches[j][0], budgetRequest._id, budgetRequest.deadLine, budgetRequest.address))
														.then((response) => {
															console.log(budgetRequest._id, groupedMatches[j][0].id)
															daoBudgetRequest.updateBudgetSupplierStatus(budgetRequest._id, groupedMatches[j][0].id, "Email enviado")
															.then((response) => {
																console.log("Email enviado para " + groupedMatches[j][0].id);
															})
															.catch((err) => {
																console.log(err);
															});
...
function getFormattedBudgetRequest(budgetRequest, groupedMatches) {
	...
	for (let i = 0; i < groupedMatches.length; i++) {
		...
		formattedMatch.supplierId = supplierInfos.id;
